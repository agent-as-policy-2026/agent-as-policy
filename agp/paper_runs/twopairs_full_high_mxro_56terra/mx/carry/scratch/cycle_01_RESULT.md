...
## Saved checkpoint and final timing
Checkpoint accepted: experience/cycle_01/memory.json. Server final metrics: task 512.465s; preparation/between-cycle 32.335s; reflection 108.914s; total 653.714s (10.90min). Motion calls 25, captures 27, failed stage attempts 0, unsuccessful commands 3 (two rejected IK moves plus one no-valid-depth query). Motion command waits 128.209s. Checkpoint preserves the report as it existed before commitment; this working report adds the returned final metrics. No motion after checkpoint.
