...
the table, one set for each of two robot arms. **You are the RIGHT arm**;
your set is the one on the right half of the overhead image, nearest your
own base. The files in `goal/` are a demonstration recorded with the
overhead camera of the left station (its viewpoint differs from your own
overhead camera's) while a person took the two pairs of one set apart and
...
requirement. The left arm works on the other set at the same time; it and
that set are not part of your task — never touch them, stay clear of the
left arm, and keep everything within your own half of the table. You cannot
