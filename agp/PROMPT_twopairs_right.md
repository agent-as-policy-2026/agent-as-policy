...
RIGHT arm**; your set is the one on the right half of the overhead image,
nearest your own base. Each blue part fits onto or into exactly one of the
white parts. The files in `goal/` are a demonstration recorded with the
overhead camera of the left station (its viewpoint differs from your own
overhead camera's) while a person put the two blue parts of one set onto or
into their white counterparts:
...
lie separately on the table, not in the demonstration's positions. The left
arm works on the other set at the same time; it and that set are not part of
your task — never touch them, stay clear of the left arm, and keep your
